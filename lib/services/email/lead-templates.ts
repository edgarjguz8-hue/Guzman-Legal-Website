...
/**
 * Email templates for the attorney-lead flow.
 *
 * The HTML/text bodies are ported verbatim from the previous inline
 * implementation in `app/api/submit-lead/route.ts` so the rendered emails are
...
/** Notification email sent to the matched attorney. */
export function attorneyLeadEmail(input: LeadEmailInput): SendEmailParams {
  const { fullName, phone, email, practiceArea, zipCode, county, legalIssue } =
    input
  const formattedLegalIssue = String(legalIssue).replace(/\n/g, "<br>")

  return {
    to: input.attorneyEmail,
...
/** Confirmation email sent to the client who submitted the request. */
export function clientConfirmationEmail(input: LeadEmailInput): SendEmailParams {
  const { fullName, email, practiceArea, county, firmName } = input
...
<div style="margin:0;padding:42px 20px;background:#f4f7fc;font-family:Arial,Helvetica,sans-serif;color:#061733;">
  <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:22px;padding:42px 34px;border:1px solid #e5eaf2;text-align:center;box-shadow:0 12px 30px rgba(6,23,51,0.08);">
    ...
    <img src="${LOGO_URL}" alt="AttorneyAbogado" style="width:110px;max-width:110px;margin:0 auto 26px;display:block;border:0;" />
